...
- [x] 3. Update Reservas tab template
  - [x] 3.1 Replace the `<!-- Results -->` block with Category_Panel (3 tiles) shown when activeCategory === null
  - [x] 3.2 Add Category_Section (back button + space cards grid) shown when activeCategory !== null
  - [x] 3.3 Lock tipo filter: replace select with read-only pill when activeCategory !== null
